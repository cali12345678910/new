export type Timings = {
  ...
  Maghrib: string; Isha: string; Midnight: string;
};
export type DayData = {
  timings: Timings;
  date: {
    readable: string;
    gregorian: { date: string; weekday: { en: string }; month: { en: string }; year: string; day: string };
...
export function nextPrayer(timings: Timings): { key: PrayerKey; at: Date; msLeft: number; prevAt: Date; intervalMs: number } {
  const now = new Date();
  ...
  for (let i = 0; i < times.length; i++) {
    if (times[i].at.getTime() > now.getTime()) {
      const prev = i === 0
        ? (() => { const p = parseTimeToday(cleanTime(timings.Isha)); p.setDate(p.getDate() - 1); return p; })()
        : times[i - 1].at;
      return {
        key: times[i].key,
        at: times[i].at,
  ...
  return { key: "Fajr", at, msLeft: at.getTime() - now.getTime(), prevAt: prev, intervalMs: at.getTime() - prev.getTime() };
}
...
export function formatTime12(hhmm: string, lang: "ar" | "en"): string {
  const [h, m] = cleanTime(hhmm).split(":").map((x) => parseInt(x, 10));
  const d = new Date();
  d.setHours(h, m, 0, 0);
  return new Intl.DateTimeFormat(lang === "ar" ? "ar-SA" : "en-US", { hour: "numeric", minute: "2-digit", hour12: true }).format(d);
}
