  ...
  head: () => ({
    meta: [
      { title: "مواقيت الصلاة | Prayer Times — Al-Maqam" },
      {
        name: "description",
        content:
          "Monthly prayer times calendar — Umm Al-Qura, MWL and more — with luxurious dark presentation.",
      },
    ],
  }),
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl gold-border bg-card p-3 flex items-center justify-between text-xs">
        ...
        <span className="gold-text font-medium">
          {method ? (lang === "ar" ? method.ar : method.en) : ""}
        </span>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-secondary/60">
              <tr>
                ...
                {PRAYER_KEYS.map((k) => (
                  <th key={k} className="px-3 py-2 text-start font-display gold-text">
            ...
            <tbody>
              ...
              {data?.map((d) => {
                const greg = new Date(
                  ...
                  new Date(`${d.date.gregorian.month.en} 1`).getMonth(),
                ...
                return (
                  <tr
                    ...
                    className={`border-t border-border/60 ${isToday ? "bg-primary/10" : ""}`}
                  >
                    <td className="px-3 py-2 whitespace-nowrap">
                      <div className="font-medium">{d.date.gregorian.day}</div>
                      <div className="text-[11px] text-muted-foreground">
                        {d.date.gregorian.weekday.en}
                      </div>
                    </td>
                    <td className="px-3 py-2 whitespace-nowrap">
                      <div className="font-medium">{d.date.hijri.day}</div>
                      <div className="text-[11px] text-muted-foreground">
                        {lang === "ar" ? d.date.hijri.month.ar : d.date.hijri.month.en}
                      </div>
                    </td>
                    {PRAYER_KEYS.map((k) => (
