...
export const Route = createFileRoute("/qibla")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Find the precise direction of the Kaaba using your live location with a luxurious golden compass." },
    ],
  }),
  component: QiblaPage,
});
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-3xl gold-border gold-gradient card-shadow p-6 text-center">
        <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">{t("qibla_dir")}</div>
        <div className="font-display text-6xl gold-text mt-1 tabular-nums">{bearing.toFixed(1)}°</div>
        ...
        <div className="mx-auto mt-6 relative size-72 sm:size-80 rounded-full gold-border-strong bg-background grid place-items-center">
          ...
          <div
            ...
            style={{ transform: `rotate(${needle}deg)`, transition: "transform 600ms cubic-bezier(.2,.8,.2,1)" }}
          >
            ...
            <div
              ...
              style={{
                top: 0,
                width: 4,
                background: "linear-gradient(to top, transparent 0%, rgba(212,175,55,0.4) 20%, #FFD700 80%, #fff8d6 100%)",
                borderRadius: 4,
                boxShadow: aligned ? "0 0 24px #FFD700, 0 0 48px rgba(255,215,0,0.6)" : "0 0 12px rgba(212,175,55,0.5)",
              }}
            />
          </div>
          ...
          <div className="absolute bottom-4 text-[10px] text-muted-foreground">
            {heading !== null ? (lang === "ar" ? `اتجاهك ${heading.toFixed(0)}°` : `Heading ${heading.toFixed(0)}°`) : ""}
          </div>
        </div>
        ...
        {needsPermission && (
          <button onClick={requestPerm} className="mt-4 rounded-full gold-fill text-black px-4 py-2 text-sm font-medium">
        ...
        {!supported && (
          <div className="mt-4 inline-flex items-center gap-2 rounded-xl bg-secondary px-3 py-2 text-xs text-muted-foreground">
            ...
            {lang === "ar" ? "البوصلة غير مدعومة في هذا الجهاز — يُعرض الاتجاه المحسوب." : "Compass not supported here — showing computed bearing."}
          </div>
        )}
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 text-xs">
          <div className="rounded-xl bg-secondary/60 gold-border p-3">
            ...
            <div className="font-mono mt-1 tabular-nums">{loc.lat.toFixed(3)}, {loc.lng.toFixed(3)}</div>
          </div>
          <div className="rounded-xl bg-secondary/60 gold-border p-3">
