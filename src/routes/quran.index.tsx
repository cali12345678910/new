...
export const Route = createFileRoute("/quran/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Read and listen to the Holy Qur'an in Uthmani script — by Surah or Juz, with bookmarks and reciter selection." },
    ],
  }),
...
function QuranIndex() {
  ...
  useEffect(() => {
    try { setBookmarks(JSON.parse(localStorage.getItem("almaqam.bookmarks") || "[]")); } catch {}
  }, []);
    ...
    <div className="space-y-4">
      <header className="rounded-3xl gold-border gold-gradient card-shadow p-5">
        <div className="flex items-center gap-3">
          <div className="grid place-items-center size-14 rounded-2xl gold-fill text-black shadow-md"><BookOpen className="size-7" /></div>
          <div>
            <h1 className="font-display text-3xl gold-text">{lang === "ar" ? "القرآن الكريم" : "The Holy Qur'an"}</h1>
            <p className="text-xs text-muted-foreground">{lang === "ar" ? "١١٤ سورة • اقرأ واستمع برسم عثماني فاخر" : "114 Surahs — luxurious Uthmani script with audio"}</p>
          </div>
        </div>
          ...
          {([
            ...
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-sm transition ${
                tab === k ? "gold-fill text-black font-medium" : "gold-border bg-card hover:bg-accent"
              }`}
            >
              {label}
            </button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2.5">
          {Array.from({ length: 30 }, (_, i) => i + 1).map((j) => (
            <div key={j} className="rounded-2xl gold-border bg-card card-shadow p-3 text-center">
              ...
              <div className="text-[11px] text-muted-foreground mt-1">{lang === "ar" ? `الجزء ${j}` : `Part ${j}`}</div>
            </div>
          ))}
          <div className="col-span-full text-center text-xs text-muted-foreground">
            {lang === "ar" ? "اضغط على سورة من تبويب «سورة» لبدء القراءة." : "Open any surah from the Surah tab to start reading."}
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="col-span-full text-center text-sm text-muted-foreground py-8">
              {tab === "bookmarks"
                ? (lang === "ar" ? "لا توجد سور محفوظة بعد." : "No bookmarked surahs yet.")
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11px] text-muted-foreground truncate">
                  {s.englishNameTranslation} · {s.numberOfAyahs} {lang === "ar" ? "آية" : "verses"} · {s.revelationType}
                </div>
              </div>
              {bookmarks.includes(s.number) && <Bookmark className="size-4 text-primary fill-current" />}
