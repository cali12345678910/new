...
import { Sun, Sunrise, SunMedium, Sunset, Moon, MoonStar, Compass, BookOpen, Sparkles, Bell, BellRing } from "lucide-react";

import { LocationBar } from "@/components/LocationBar";
import { loadLocation, qiblaBearing, type Loc } from "@/lib/location";
import { fetchToday, formatTime12, nextPrayer, formatHMS, PRAYER_KEYS, type PrayerKey } from "@/lib/prayer-times";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Al-Maqam: a luxury Islamic platform for accurate prayer times, Qibla, Qur'an with reciter selection, and Hisn al-Muslim dhikr." },
    ],
  }),
  component: Dashboard,
});
...
const ICONS: Record<PrayerKey, React.ComponentType<{ className?: string }>> = {
  Fajr: Sunrise, Sunrise: Sun, Dhuhr: SunMedium, Asr: Sun, Maghrib: Sunset, Isha: Moon,
};
...
function Dashboard() {
  ...
  async function toggleAthan() {
    if (athanOn) { setAthanOn(false); stopAthan(); return; }
    const perm = await requestNotificationPermission();
    if (perm === "granted") { setAthanOn(true); playAthan(); setTimeout(stopAthan, 4000); }
    else alert(lang === "ar" ? "يرجى السماح بالتنبيهات" : "Please allow notifications");
  }

  const todayDhikr = DHIKR[0].items[1];
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-3xl gold-border gold-gradient card-shadow p-5 sm:p-7 overflow-hidden relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-6 items-center relative">
          <div className="min-w-0 order-2 md:order-1">
            <div className="text-[11px] uppercase tracking-[0.3em] text-muted-foreground">{t("next_prayer")}</div>
            ...
            <div className="mt-5 flex items-center gap-2 flex-wrap">
              <div className="text-xs uppercase tracking-widest text-muted-foreground">{t("hijri")}</div>
              <div className="font-display text-base">
                {data ? `${data.date.hijri.day} ${data.date.hijri.month.ar} ${data.date.hijri.year}` : "—"}
              </div>
            </div>
            <div className="text-xs text-muted-foreground">
              {data ? `${data.date.gregorian.weekday.en}, ${data.date.gregorian.day} ${data.date.gregorian.month.en} ${data.date.gregorian.year}` : "—"}
            </div>

            <button
              onClick={toggleAthan}
              className={`mt-5 inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm transition ${
                athanOn ? "gold-fill text-black animate-gold-pulse" : "bg-background/70 gold-border hover:bg-accent"
              }`}
            >
          ...
          <div className="order-1 md:order-2 grid place-items-center">
            <RadialCountdown size={240} stroke={12} progress={progress}>
              <div>
                <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">{t("in_time")}</div>
          ...
          return (
            <div
              ...
              className={`rounded-2xl p-3 transition card-shadow ${
                isNext ? "gold-fill text-black border-transparent animate-gold-pulse" : "bg-card gold-border"
              }`}
            >
              <div className="flex items-center justify-between">
                <Icon className="size-4 opacity-80" />
                {isNext && <span className="text-[10px] uppercase tracking-wider opacity-80">●</span>}
              </div>
              <div className="mt-2 font-display text-lg leading-tight">{t(k.toLowerCase() as any)}</div>
              <div className={`text-sm font-mono tabular-nums ${isNext ? "" : "text-muted-foreground"}`}>
        ...
        {data && (
          <div className="rounded-2xl gold-border bg-card p-3 col-span-2 sm:col-span-3 lg:col-span-6">
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1.5"><MoonStar className="size-3.5 text-primary" /> {t("midnight")}</span>
              <span className="font-mono tabular-nums text-foreground">{formatTime12(data.timings.Midnight, lang)}</span>
            </div>
          </div>
        )}
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Link to="/qibla" className="rounded-3xl gold-border bg-card card-shadow p-5 hover:-translate-y-0.5 transition group">
          <div className="flex items-center justify-between">
            <div className="text-xs uppercase tracking-widest text-muted-foreground">{t("qibla_dir")}</div>
        ...
        <Link to="/dhikr" className="rounded-3xl gold-border bg-card card-shadow p-5 hover:-translate-y-0.5 transition">
          <div className="flex items-center justify-between">
            <div className="text-xs uppercase tracking-widest text-muted-foreground">{t("today_dhikr")}</div>
            <Sparkles className="size-4 text-primary" />
          </div>
          <p className="mt-3 arabic text-xl leading-relaxed line-clamp-4">{todayDhikr.ar}</p>
          {lang === "en" && todayDhikr.en && <p className="text-xs text-muted-foreground mt-2 line-clamp-2">{todayDhikr.en}</p>}
        </Link>

        <Link to="/quran" className="rounded-3xl gold-border bg-card card-shadow p-5 hover:-translate-y-0.5 transition">
          <div className="flex items-center justify-between">
            <div className="text-xs uppercase tracking-widest text-muted-foreground">{lang === "ar" ? "آية اليوم" : "Ayah of the day"}</div>
            <BookOpen className="size-4 text-primary" />
          </div>
          <p className="mt-3 arabic text-xl leading-relaxed">{AYAH_OF_DAY.ar}</p>
  ...
  return (
    <div className="mt-4 mx-auto relative size-28 rounded-full gold-border bg-background grid place-items-center">
      ...
      <div
        ...
        style={{ transform: `rotate(${bearing}deg)`, background: "linear-gradient(to top, transparent, var(--gold-bright))" }}
      />
...
function Ornament() {
  return (
    <svg viewBox="0 0 200 200" className="pointer-events-none absolute -end-12 -top-12 size-56 opacity-20 text-primary" fill="none" stroke="currentColor" strokeWidth="0.6">
